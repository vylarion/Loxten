<script lang="ts">
  ...
  import { Shield, Bell, ChartBar, GearSix, ArrowCounterClockwise, Trash, EnvelopeSimple, MagnifyingGlass, Plus, X, Globe, Info } from 'phosphor-svelte';
  ...
  async function clearData(): Promise<void> {
    if (confirm('This will clear all saved scan history and statistics. Continue?')) {
      try {
        ...
        stats = { sitesScanned: 0, threatsBlocked: 0, trackersBlocked: 0, malwareDetected: 0, phishingBlocked: 0 };
        breachResult = null;
      } catch (error) {
  ...
  <section>
    ...
    <div class="toggle-list">
      <label class="toggle-row">
        ...
        <input type="checkbox" class="toggle" bind:checked={settings.realTimeProtection} on:change={handleToggle} />
      </label>
      <label class="toggle-row">
        <span class="toggle-label">Block Malicious Sites</span>
        <input type="checkbox" class="toggle" bind:checked={settings.blockMaliciousSites} on:change={handleToggle} />
      </label>
      <label class="toggle-row">
        <span class="toggle-label">Block Phishing</span>
        <input type="checkbox" class="toggle" bind:checked={settings.blockPhishing} on:change={handleToggle} />
      </label>
      <label class="toggle-row">
        <span class="toggle-label">Block Trackers</span>
        <input type="checkbox" class="toggle" bind:checked={settings.blockTrackers} on:change={handleToggle} />
      </label>
      <label class="toggle-row">
        <span class="toggle-label">Block Cryptominers</span>
        <input type="checkbox" class="toggle" bind:checked={settings.blockCryptominers} on:change={handleToggle} />
      </label>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="toggle-list">
      <label class="toggle-row">
        ...
        <input type="checkbox" class="toggle" bind:checked={settings.showWarnings} on:change={handleToggle} />
      </label>
    </div>
    <div class="segment-group">
      <span class="segment-label">Alert Level</span>
      <div class="segment">
        <button class="seg-btn" class:active={settings.notificationLevel === 'low'} on:click={() => setNotificationLevel('low')}>Low</button>
        <button class="seg-btn" class:active={settings.notificationLevel === 'medium'} on:click={() => setNotificationLevel('medium')}>Med</button>
        <button class="seg-btn" class:active={settings.notificationLevel === 'high'} on:click={() => setNotificationLevel('high')}>High</button>
      </div>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="toggle-list">
      <label class="toggle-row">
        ...
        <input type="checkbox" class="toggle" bind:checked={settings.autoScan} on:change={handleToggle} />
      </label>
    </div>
    <div class="segment-group">
      <span class="segment-label">Frequency</span>
      <div class="segment">
        <button class="seg-btn" class:active={settings.scanFrequency === 'realtime'} on:click={() => setScanFrequency('realtime')}>Live</button>
        <button class="seg-btn" class:active={settings.scanFrequency === 'periodic'} on:click={() => setScanFrequency('periodic')}>Periodic</button>
        <button class="seg-btn" class:active={settings.scanFrequency === 'manual'} on:click={() => setScanFrequency('manual')}>Manual</button>
      </div>
    </div>
  </section>
  ...
  <section>
    ...
    <div class="breach-input-row">
      <input 
        ...
        on:keydown={(e) => { if (e.key === 'Enter') checkBreach(); }}
      />
      <button class="icon-btn" on:click={checkBreach} disabled={breachChecking || !breachEmail.trim()}>
        {#if breachChecking}
    ...
    {#if breachResult}
      <div class="breach-result">
        {#if breachResult.breached}
          <span class="breach-warn">Found in {breachResult.breachCount} breach{breachResult.breachCount !== 1 ? 'es' : ''}</span>
        {:else}
